...
Este guia detalha o **passo a passo de implementação** para estruturar a organização de recursos no Azure, visando o **rastreamento de custos** e a **responsabilidade financeira** alinhada à estrutura de negócios da empresa.

## Cenário e Requisitos Chave

O cenário exige uma estrutura que reflita a organização da empresa (2 Unidades de Negócios, cada uma com 3 Departamentos) para que cada subunidade possa acompanhar seus próprios gastos.

| Requisito | Objetivo de Implementação | Ferramenta Chave |
| :--- | :--- | :--- |
| **Estrutura de Negócios** | Espelhar a organização da empresa (Unidades/Departamentos). | Grupos de Gerenciamento (Management Groups) |
| **Responsabilidade de Custos** | Isolar e rastrear custos por Unidade de Negócio e Departamento. | Assinaturas (Subscriptions) e Tags |
| **Relatórios Corporativos** | Gerar relatórios de custos consolidados para a empresa. | Azure Cost Management |

## Passo a Passo de Implementação (Azure Management Groups)

A solução mais robusta envolve a criação de uma **hierarquia de Grupos de Gerenciamento (Management Groups - MG)** que alinha a governança e o faturamento com a estrutura organizacional.

### Fase 1: Desenho da Hierarquia de Governança

A melhor prática é a **Hierarquia Rasa (Focada em Unidades de Negócio)**, que usa Grupos de Gerenciamento para as Unidades e Grupos de Recursos/Tags para os Departamentos.

| Nível da Hierarquia | Entidade | Propósito |
| :--- | :--- | :--- |
| **Nível 0 (Raiz)** | `Tenant Root Group` | Ponto de aplicação de políticas globais (ex: Proibir região). |
| **Nível 1 (Empresa)** | `Tailwind Traders (MG)` | Consolidação de todos os custos e aplicação de políticas corporativas. |
| **Nível 2 (Unidades)** | `Vestuário (MG)` e `Artigos Esportivos (MG)` | Limite de governança e faturamento para cada Unidade de Negócio. |
| **Nível 3 (Faturamento)** | `Assinatura-Vestuário (Sub)` e `Assinatura-Artigos-Esportivos (Sub)` | Onde os recursos são implantados e os custos são acumulados. |
| **Nível 4 (Departamentos)** | `rg-vest-marketing (RG)` e outros | Agrupamento lógico de recursos e rastreamento de custos departamentais. |

### Fase 2: Implementação da Estrutura

| Etapa | Ação | Detalhes da Implementação | Ferramenta/Serviço |
| :--- | :--- | :--- | :--- |
| **2.1** | **Criação dos Grupos de Gerenciamento** | Crie os MGs `Tailwind Traders`, `Vestuário` e `Artigos Esportivos`, aninhando-os sob o MG raiz. | Azure Management Groups |
| **2.2** | **Criação e Atribuição de Assinaturas** | Crie as assinaturas (ex: `Assinatura-Vestuário`) e mova-as para o MG da respectiva Unidade de Negócio. **Isso garante que o faturamento seja isolado por Unidade.** | Azure Subscriptions |
| **2.3** | **Definição de Tags Obrigatórias** | Use o **Azure Policy** no nível do MG `Tailwind Traders` para exigir que todos os recursos tenham as tags `UnidadeDeNegocio` e `Departamento`. | Azure Policy (Efeito `Deny` ou `Append`) |
| **2.4** | **Criação de Grupos de Recursos** | Dentro de cada Assinatura, crie Grupos de Recursos para cada Departamento (ex: `rg-vestuario-marketing`). | Azure Resource Groups |

### Fase 3: Rastreamento e Relatórios de Custos

| Etapa | Ação | Detalhes da Implementação | Ferramenta/Serviço |
| :--- | :--- | :--- | :--- |
| **3.1** | **Relatório Corporativo (Visão Consolidada)** | A equipe de TI central usa o **Azure Cost Management** e filtra os custos pelo **Grupo de Gerenciamento** `Tailwind Traders` para obter o custo total da empresa. | Azure Cost Management |
| **3.2** | **Relatório de Unidade de Negócio** | A liderança de `Vestuário` usa o **Azure Cost Management** e filtra pelo **Grupo de Gerenciamento** `Vestuário` (ou pela `Assinatura-Vestuário`) para ver o custo total de sua unidade. | Azure Cost Management |
| **3.3** | **Relatório Departamental** | O gerente de `Marketing` da unidade `Vestuário` usa o **Azure Cost Management** e filtra pelo **Grupo de Recursos** `rg-vestuario-marketing` ou pela **Tag** `Departamento:Marketing` para ver seus gastos específicos. | Azure Cost Management |

## Justificativa para a Certificação

A **Hierarquia Focada em Unidades de Negócio** é a mais indicada porque:

1.  **Alinhamento Financeiro:** Alinha a estrutura de governança do Azure (MGS e Assinaturas) diretamente com a estrutura de responsabilidade financeira da empresa.
2.  **Governança por Herança:** Políticas de alto nível (ex: segurança) são aplicadas no MG `Tailwind Traders` e herdadas por todos. Políticas específicas (ex: tipos de VM permitidos) são aplicadas nos MGs de Unidade de Negócio.
3.  **Simplicidade:** Evita uma hierarquia de MGs muito profunda (Hierarquia Profunda), usando o **Grupo de Recursos** e **Tags** para a granularidade final (Departamentos), o que é mais flexível e fácil de gerenciar.
...
| Requisito | Objetivo de Implementação | Ferramenta Chave |
| :--- | :--- | :--- |
| **Escalabilidade e Alta Disponibilidade** | A aplicação deve lidar com picos de tráfego e estar sempre online. | Azure App Service (Autoscale e Slots) |
| **Entrega de Conteúdo Rápida** | O conteúdo estático (imagens, CSS, JS) deve ser entregue rapidamente aos usuários em qualquer lugar do mundo. | Azure Content Delivery Network (CDN) |
| **Ambiente de Desenvolvimento/Teste** | Necessidade de um ambiente de *staging* para testes antes da produção. | Azure App Service Deployment Slots |
...
| Etapa | Ação | Detalhes da Implementação | Ferramenta/Serviço |
| :--- | :--- | :--- | :--- |
| **1.1** | **Escolha do Serviço de Hospedagem** | Selecione o **Azure App Service** (Plano *Standard* ou superior) para hospedar a aplicação de e-commerce. O plano deve suportar *Deployment Slots* e *Autoscale*. | Azure App Service |
...
| **1.4** | **Implantação e Troca (*Swap*)** | Implante o código no *slot* `staging` para testes. Após a validação, use a função **Swap** para trocar o `staging` pelo `production` (troca instantânea de tráfego com *warm-up*). | Azure DevOps / GitHub Actions / Azure Portal |
